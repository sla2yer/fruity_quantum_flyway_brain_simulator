...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Milestone 14 dashboard integration verification pass and publish a readiness report."
    )
    parser.add_argument(
        "--config",
        required=True,
        help="Path to the YAML config file that defines the local Milestone 14 verification outputs.",
    )
    parser.add_argument(
        "--fixture-test-target",
        action="append",
        ...
        default=[],
        help="Optional unittest target to add to the focused Milestone 14 verification suite. Repeatable.",
    )
    parser.add_argument(
        "--skip-fixture-tests",
        action="store_true",
        help="Skip the focused Milestone 14 dashboard fixture verification suite.",
    )
    ...
    summary = execute_milestone14_readiness_pass(
        ...
        json.dumps(
            {
                ...
                "dashboard_session_metadata_path": summary["workflow_audit"][
                    "build_audit"
                ].get("metadata_path", ""),
                "dashboard_app_shell_path": summary["workflow_audit"]["build_audit"].get(
                    "app_shell_path", ""
                ),
            },
            indent=2,
