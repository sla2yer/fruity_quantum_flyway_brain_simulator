...
from flywire_wave.milestone8a_readiness import (
    DEFAULT_FIXTURE_TEST_TARGETS,
    execute_milestone8a_readiness_pass,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Milestone 8A integration verification pass and publish a readiness report."
    )
    parser.add_argument(
        "--config",
        required=True,
        help="Path to the YAML config file that defines the local Milestone 8A verification outputs.",
    )
    parser.add_argument(
        "--fixture-test-target",
        action="append",
        ...
        default=[],
        help="Optional unittest target to add to the focused Milestone 8A verification suite. Repeatable.",
    )
    parser.add_argument(
        "--skip-fixture-tests",
        action="store_true",
        help="Skip the focused Milestone 8A fixture verification suite.",
    )
    ...
    summary = execute_milestone8a_readiness_pass(
