...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Milestone 10 integration verification pass and publish a readiness report."
    )
    parser.add_argument(
        "--config",
        required=True,
        help="Path to the YAML config file that defines the local Milestone 10 verification outputs.",
    )
    parser.add_argument(
        "--fixture-test-target",
        action="append",
        ...
        default=[],
        help="Optional unittest target to add to the focused Milestone 10 verification suite. Repeatable.",
    )
    parser.add_argument(
        "--skip-fixture-tests",
        action="store_true",
        help="Skip the focused Milestone 10 fixture verification suite.",
    )
    ...
    summary = execute_milestone10_readiness_pass(
