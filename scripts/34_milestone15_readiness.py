...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the Milestone 15 experiment-orchestration integration verification "
            "pass and publish a readiness report."
        )
    )
    parser.add_argument(
        "--config",
        required=True,
        help="Path to the YAML config file that defines the local Milestone 15 verification outputs.",
    )
    parser.add_argument(
        "--fixture-test-target",
        action="append",
        ...
        default=[],
        help="Optional unittest target to add to the focused Milestone 15 verification suite. Repeatable.",
    )
    parser.add_argument(
        "--skip-fixture-tests",
        action="store_true",
        help="Skip the focused Milestone 15 orchestration fixture verification suite.",
    )
    ...
    summary = execute_milestone15_readiness_pass(
        ...
        json.dumps(
            {
                ...
                READY_FOR_FOLLOW_ON_WORK_KEY: summary[FOLLOW_ON_READINESS_KEY][
                    READY_FOR_FOLLOW_ON_WORK_KEY
                ],
                "markdown_path": summary["markdown_path"],
                "json_path": summary["json_path"],
                "shuffle_suite_state_path": (
                    summary["shuffle_simulation_audit"]["command"]
                    .get("parsed_summary", {})
                    .get("state_path", "")
                ),
                "suite_review_summary_path": summary["review_audit"]["report_audit"][
                    "summary_path"
                ],
            },
            indent=2,
