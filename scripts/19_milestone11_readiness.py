...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Milestone 11 mixed-fidelity integration verification pass and publish a readiness report."
    )
    parser.add_argument(
        "--config",
        required=True,
        help="Path to the YAML config file that defines the local Milestone 11 verification outputs.",
    )
    parser.add_argument(
        "--fixture-test-target",
        action="append",
        ...
        default=[],
        help="Optional unittest target to add to the focused Milestone 11 verification suite. Repeatable.",
    )
    parser.add_argument(
        "--skip-fixture-tests",
        action="store_true",
    ...
    summary = execute_milestone11_readiness_pass(
