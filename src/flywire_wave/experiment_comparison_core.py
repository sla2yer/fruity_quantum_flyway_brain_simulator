...
def _subset_analysis_plan(
    ...
    *,
    ...
    active_metric_ids = {str(metric_id) for metric_id in metric_ids}
    return {
    ...
    *,
    ...
    while changed:
        ...
        for group_id, group in groups_by_id.items():
            ...
            changed = True
    missing_group_ids = sorted(set(groups_by_id) - set(seeds_by_group_id))
...
def _build_matched_pair_detail_rows(
    *,
    ...
    metric_rows_by_arm_seed_key: Mapping[
        tuple[str, int],
...
def _evaluate_null_tests(
    *,
    ...
    shared_metric_rows = [
        copy.deepcopy(dict(item)) for item in shared_result.get("metric_rows", [])
    ]
    task_metric_rows = [
        copy.deepcopy(dict(item)) for item in task_result.get("metric_rows", [])
    ]
            ...
            metric_outcomes.append(
                {
                    ...
                    "delta_from_reference_mean": rollup["component_statistics"][
    ...
    for group_id in declaration["comparison_group_ids"]:
        for metric_id in declaration["required_metric_ids"]:
            ...
            if not seed_rows:
                ...
                continue
            nonzero_signs = {
                _value_sign(float(item["value"]))
                for item in seed_rows
                if _value_sign(float(item["value"])) != "zero"
            }
...
def _build_output_summaries(
    *,
    ...
    for output_id in analysis_plan["active_output_ids"]:
        normalized_output_id = str(output_id)
...
__all__ = ["compute_experiment_comparison_summary"]
